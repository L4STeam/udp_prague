...
class PragueSender {
public:
  PragueSender(AppStuff &app, PragueCC &cc);
  void run() {
    init();
    while (true) {
      sendPackets();
...
int main(int argc, char **argv) {
  ...
  struct ackmessage_t &ack_msg =
      (struct ackmessage_t &)(receivebuffer); // overlaying the receive buffer
  struct datamessage_t &data_msg =
      (struct datamessage_t &)(sendbuffer); // overlaying the send buffer

  // Frame message
  struct framemessage_t &frame_msg =
      (struct framemessage_t &)(sendbuffer); // overlaying the send buffer

  // RFC8888 buffer
  struct rfc8888ack_t &rfc8888_ackmsg =
      (struct rfc8888ack_t &)(receivebuffer); // overlaying the receive buffer
  ...
  count_tp seqnr = 0;     // sequence number of last sent packet (first sequence
                          // number will be 1)
  count_tp inflight = 0;  // packets in-flight counter
  rate_tp pacing_rate;    // used for pacing the packets with the right interval
                          // (not taking into account the bursts)
  count_tp packet_window; // allowed maximum packets in-flight
  count_tp packet_burst;  // allowed number of packets to send back-to-back;
                          // pacing interval needs to be taken into account for
                          // the next burst only
  size_tp packet_size; // packet size is reduced when rates are low to preserve
                       // 2 packets per 25ms pacing interval
  ...
  count_tp frame_nr = 0;   // frame sequence number of last sent frame (first
                           // frame sequence number will be 1)
    ...
    if (!app.rt_mode) {
      ...
      if (startSend != 0) {
        if (compRecv + packet_size * inburst * 1000000 / pacing_rate <= 0)
          ...
          nextSend = time_tp(startSend + compRecv +
