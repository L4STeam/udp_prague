...
struct AppStuff {
    ...
    AppStuff(bool sender, int argc, char **argv):
        sender_role(sender), verbose(false), quiet(false), rcv_addr("0.0.0.0"), rcv_port(8080), connect(false), max_pkt(1500), data_tm(1), ack_tm(1), rept_tm(1000000),
        acc_bytes_sent(0), acc_bytes_rcvd(0), acc_rtts(0), count_rtts(0), prev_packets(0), prev_marks(0), prev_losts(0)
    {
        ...
        if (verbose) {
            if (sender_role) {
                printf("s: time, timestamp, echoed_timestamp, time_diff, seqnr, packet_size,,,,, pacing_rate, packet_window, packet_burst, inflight, inburst, nextSend\n");
                printf("r: time, timestamp, echoed_timestamp, time_diff, seqnr, bytes_received, packets_received, packets_CE, packets_lost, error_L4S,,,,, inflight, inburst, nextSend\n");
            } else {
    ...
    {
        if (verbose) {
            // "s: time, timestamp, echoed_timestamp, time_diff, seqnr, packet_size,,,,, pacing_rate, packet_window, packet_burst, inflight, inburst, nextSend"
    ...
    {
        if (verbose) {
            // "r: time, timestamp, echoed_timestamp, time_diff, seqnr, bytes_received, packets_received, packets_CE, packets_lost, error_L4S,,,,, inflight, inburst, nextSend"
        ...
        if (!quiet) {
            ...
            if (now - rept_tm >= 0) {
                ...
                printf("[SENDER]: %.2f sec, Sent: %.3f Mbps, Rcvd: %.3f Mbps, RTT: %.3f ms, Mark: %.2f%%(%d/%d), Lost: %.2f%%(%d/%d), Pacing rate: %f Mbps, InFlight/W: %d/%d packets, InBurst/B: %d/%d packets\n",
