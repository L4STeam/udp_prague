...
public:
  UDPSocket()
    ...
    if (SOCKET_ERROR == WSAIoctl(sockfd, SIO_GET_EXTENSION_FUNCTION_POINTER,
                                 ...
                                 &WSARecvMsg, sizeof(WSARecvMsg), &dwBytes,
                                 NULL, NULL)) {
    ...
    if (SOCKET_ERROR == WSAIoctl(sockfd, SIO_GET_EXTENSION_FUNCTION_POINTER,
                                 ...
                                 &WSASendMsg, sizeof(WSASendMsg), &dwBytes,
                                 NULL, NULL)) {
    ...
    DWORD enabled = TRUE;
    if ((SOCKET_ERROR == setsockopt(sockfd, IPPROTO_IP, IP_RECVTOS,
                                    (char *)&enabled, sizeof(enabled))) &&
        (SOCKET_ERROR == setsockopt(sockfd, IPPROTO_IPV6, IPV6_RECVTCLASS,
                                    (char *)&enabled, sizeof(enabled)))) {
  ...
  size_tp Receive(char *buf, size_tp len, ecn_tp &ecn, time_tp timeout) {
    ...
    if (timeout > 0) {
      if (timeout < 15000)
        timeout = 0; // Spin for Windows. Select without blocking if the timeout
                     // is smaller than 15ms
  ...
  size_tp Send(char *buf, size_tp len, ecn_tp ecn) {
    ...
    cmsg->cmsg_level = (PSOCKADDR_STORAGE(&peer_addr)->ss_family == AF_INET)
                           ? IPPROTO_IP
                           : IPPROTO_IPV6;
    cmsg->cmsg_type = (PSOCKADDR_STORAGE(&peer_addr)->ss_family == AF_INET)
                          ? IP_ECN
                          : IPV6_ECN;
    *(PINT)WSA_CMSG_DATA(cmsg) = ecn;
    ...
    if (current_ecn != ecn) {
      ...
      if (setsockopt(sockfd, IPPROTO_IP, IP_TOS, &ecn_set, sizeof(ecn_set)) <
          0) {
